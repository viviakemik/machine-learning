...
def traditional_pivot(df_1,col_high, col_low, col_close):
    ...
    """
    Used to calculate the traditional pivot values

    Parameters
    ----------
    df_1 : Dataframe
    Data on which the pivots value needs to be calculated
...
def woodie_pivot(df_1, col_high, col_low, col_close):
    """
        Calculates the woodie pivot values
...
def camarilla_pivot(df_1, col_high, col_low, col_close):
    """
        Used to calculate the camarilla pivot values
...
def resample_ohlc(raw_data, col_names, timeframe):
    """
        ...
        ----------
        raw_data : Dataframe
        Raw Data which is to be resampled

        col_names: list
        [open_column_name, high_column_name, low_column_name, close_column_name]
...
def merge_ohlc_pivot(ohlc_data, pivot_data):
    """
        Merge ohlc data and pivot values

        Parameters
        ----------
        ohlc_data : Dataframe
        Dataframe with ohlc values used for trading

        pivot_data: Dataframe
        Dataframe with pivot values


        Returns
        -------
        DataFrame
        Dataframe with merged ohlc data and pivot data
        """
...
def generate_buy_pivot_exit(merged_data_1,close_col,open_col):
    """
            Generate Buy signals with stoploss as the entry pivot level

            Parameters
            ----------
            merged_data_1 : Dataframe
            Dataframe with ohlc values used for trading
...
def generate_sell_pivot_exit(merged_data_1,close_col,open_col):
    """
        Generate Sell signals with stoploss as the entry pivot level
...
def generate_buy_with_supertrend(merged_data_1, ticker_name,close_col, open_col):
    """
        Generate Buy signals only when the supertrend is in buy side and stoploss as 0.005%  and profit as 0.01% from the entry value
        ...
        ticker_name: str
        The stock ticker on which the Trades needed to be generated
...
def generate_sell_with_supertrend(merged_data_1,ticker_name,close_col,open_col):
    """
        Generate Sell signals only when the supertrend is in buy side and stoploss as 0.005%  and profit as 0.01% from the entry value
